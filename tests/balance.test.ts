...
describe('баланс тримається на посередній грі', () => {
  for (let map = 0; map < MAPS.length; map++) {
    for (const mode of [MODE_FIXED, MODE_MAZE]) {
      ...
      /* Лише для фіксованого шляху. Лабіринт свідомо лишений легким —
         там гравець сам крутить трасу, і бот проходить стелю прогону
         майже без втрат. Це рішення автора, а не недогляд, тож ця
         сторожа туди не застосовується. */
      if (mode !== MODE_FIXED) continue;
